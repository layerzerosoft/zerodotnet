...
var logger = host.Services.GetRequiredService<ILoggerFactory>().CreateLogger("LayerZero.Fulfillment.Kafka.Bootstrap");

logger.LogInformation("Fulfillment migrations started.");
await KafkaFulfillmentBootstrapHost.ApplyMigrationsAsync(host.Services).ConfigureAwait(false);
...
await KafkaFulfillmentBootstrapHost.ProvisionMessagingAsync(host.Services).ConfigureAwait(false);
