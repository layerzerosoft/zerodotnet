...
public sealed class ZeroRuleBuilder<T, TProperty>
{
    private readonly Action<ZeroRule<T>> addRule;
    private readonly Func<T, TProperty> accessor;
    private readonly string propertyName;

    internal ZeroRuleBuilder(string propertyName, Func<T, TProperty> accessor, Action<ZeroRule<T>> addRule)
    {
    ...
    public ZeroRuleBuilder<T, TProperty> NotNull(string? message = null, string code = ZeroValidationCodes.NotNull)
    {
        return AddSyncRule(
    ...
    /// <param name="code">Optional custom failure code.</param>
    /// <returns>The current rule builder.</returns>
    public ZeroRuleBuilder<T, TProperty> NotEmpty(string? message = null, string code = ZeroValidationCodes.NotEmpty)
    {
        return AddSyncRule(
            HasValue,
            code,
    ...
    /// <param name="code">Optional custom failure code.</param>
    /// <returns>The current rule builder.</returns>
    public ZeroRuleBuilder<T, TProperty> MaximumLength(
        int maximum,
        string? message = null,
        string code = ZeroValidationCodes.MaximumLength)
    {
    ...
    /// <param name="code">Optional custom failure code.</param>
    /// <returns>The current rule builder.</returns>
    public ZeroRuleBuilder<T, TProperty> MinimumLength(
        int minimum,
        string? message = null,
        string code = ZeroValidationCodes.MinimumLength)
    {
    ...
    /// <param name="code">Optional custom failure code.</param>
    /// <returns>The current rule builder.</returns>
    public ZeroRuleBuilder<T, TProperty> Must(
        Func<TProperty, bool> predicate,
        string message,
        string code = ZeroValidationCodes.Must)
    {
    ...
    /// <param name="code">Optional custom failure code.</param>
    /// <returns>The current rule builder.</returns>
    public ZeroRuleBuilder<T, TProperty> MustAsync(
        ...
        string code = ZeroValidationCodes.Must)
    {
        ...
        addRule(new ZeroRule<T>(async (instance, _, cancellationToken) =>
        {
            TProperty value = accessor(instance);
            bool isValid = await predicate(value, cancellationToken).ConfigureAwait(false);
            return isValid ? null : new ZeroValidationFailure(propertyName, code, message, value);
        }));

        return this;
    }

    private ZeroRuleBuilder<T, TProperty> AddSyncRule(
        Func<TProperty, bool> predicate,
        string code,
        string message)
    {
        ...
        addRule(new ZeroRule<T>((instance, _, _) =>
        {
            TProperty value = accessor(instance);
            return ValueTask.FromResult(predicate(value)
                ? null
                : new ZeroValidationFailure(propertyName, code, message, value));
        }));
