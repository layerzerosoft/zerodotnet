...
public abstract class ZeroValidator<T> : IZeroValidator<T>
{
    private readonly List<ZeroRule<T>> rules = [];
    ...
    public ValueTask<ZeroValidationResult> ValidateAsync(T instance, CancellationToken cancellationToken = default)
    {
        return ValidateAsync(instance, ZeroValidationContext.Empty, cancellationToken);
    }

    /// <inheritdoc />
    public async ValueTask<ZeroValidationResult> ValidateAsync(
        T instance,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (ZeroRule<T> rule in rules)
        {
            cancellationToken.ThrowIfCancellationRequested();

            ZeroValidationFailure? failure = await rule
    ...
    protected ZeroRuleBuilder<T, TProperty> RuleFor<TProperty>(string propertyName, Func<T, TProperty> accessor)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(propertyName);
        ArgumentNullException.ThrowIfNull(accessor);

        return new ZeroRuleBuilder<T, TProperty>(propertyName, accessor, rules.Add);
    }
}
