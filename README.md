...
- Public packages: `LayerZero.Core`, `LayerZero.Validation`,
  ...
  `LayerZero.Messaging.Nats`, `LayerZero.Migrations`,
  `LayerZero.Migrations.SqlServer`, `LayerZero.Testing`, and
  `LayerZero.Client`.
...
- `LayerZero.Migrations`: transport-neutral relational migration runtime, forward-only migration DSL, seed profiles, validation, baselining, and source-generated migration discovery.
- `LayerZero.Migrations.SqlServer`: SQL Server adapter, SQL generation, schema history storage, `sp_getapplock` runner coordination, and explicit execution flows.
...
LayerZero relational migrations are code-first, forward-only, and
source-generated by default:

```csharp
builder.Services
    .AddLayerZeroMigrations()
...
Migration and seed artifacts are normal classes:
...
The runtime surface is explicit: `InfoAsync`, `ValidateAsync`, `ScriptAsync`,
...
opt-in. The current SQL Server foundation is documented in
